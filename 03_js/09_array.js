...
// 배열의 맨 앞에 데이터 추가(push) 및 제거(pop)
arr.push(10); //배열의 마지막에 요소를 추가, 원본 배열이 변경된다.
console.log(arr);
let popeNum = arr.pop(); // 배열의 마지막 요소를 제거하고 그 값을 반환한다. 원본 배열이 변경된다.
console.log(arr, popeNum);
console.log("-------shift, unshift------");
//배열에 맨 앞에 데이터 추가(unshift) 및 제거(shift)
arr.unshift(0);
console.log(arr);
let shiftNum = arr.shift(); // 배열의 첫 번째 요소를 제거하고 그 값을 반환한다. 원본 배열이 변경된다.
console.log(arr, shiftNum);

console.log("-------splice------");
//splice : 배열의 중간요소를 제거 및 추가.
let spliceNum = arr.splice(0, 3, 1, 4); // 0번째 3개 삭제, 그 값을 배열로 반환, 원본 배열이 변경된다. 세번 째 인자부터는 배열에 추가한다.
...
filter()
-각 요소를 순회하며 콜백함수를 실행하며, 특정 조건을 만족하는 요소만을 새로운 배열로 모아서 변환한다.
-콜백 함수의 return 값이 boolean(true, false)값이어야 한다.
-기본 배열은 변경되지 않는다.

map()
-각 요소를 순회하며 콜백함수를 실행하며, 요소의 값을 통해서 새로운 값을 만들어 내어 새로운 배열로 모아서 반환한다.
-콜백함수의 return값이 곧 새로운 배열의 요소가 된다.
*/
let userList = [
