...
콜백함수 (callback)
-다른 함수에 인자로 전달되는 함수
=> 함수의 실행결과를 가지고 다른 함수를 실행해야할 때
-js에서는 함수를 값으로 평가하기 때문에 가능하다.
=> 콜백함수로써 값을 전달할 때는 호출하는 형태가 되면 안된다.
=> 콜백함수를 직접 호출해야되는 경우 익명함수로 감싸서 그 안에서 호출하면 된다.
-콜백 함수로 전달되는 인자로는 화살표 함수를 많이 사용한다.
*/
...
//콜백 함수는 값으로 전달해야되기 때문에 익명함수 안에서 호출하는 형태.
...
/* 
num03을 사용해서 숫자 5,7을 더한 값을 7번 출력하기
